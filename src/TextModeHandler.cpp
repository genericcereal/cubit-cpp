...
TextModeHandler::TextModeHandler(ElementModel* elementModel,
...
{
    qDebug() << "TextModeHandler::onPress() at" << x << y;
    ...
    auto command = std::make_unique<CreateDesignElementCommand>(
        ...
        CreateDesignElementCommand::TextElement, rect,
...
void TextModeHandler::onMove(qreal x, qreal y)
{
    ...
    // Get the selected element (the Text element that was just created)
