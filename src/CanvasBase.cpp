...
CanvasBase::CanvasBase(QWidget *parent) : QWidget(parent), mode("Select") {
...
void CanvasBase::resizeEvent(QResizeEvent *event) {
    QWidget::resizeEvent(event);
}

void CanvasBase::showControls(const QRect &rect) {
    if (controls) {
...
void CanvasBase::hideControls() {
    if (controls) {
        controls->hide();
    }
}

void CanvasBase::selectElement(const QString &elementId, bool addToSelection) {
    if (!addToSelection) {
...
void CanvasBase::updateControlsVisibility() {
...
void CanvasBase::setMode(const QString &newMode) {
    if (mode != newMode) {
        mode = newMode;
...
void CanvasBase::createVariable() {
    // Generate ID based on number of elements + 1
...
void CanvasBase::mousePressEvent(QMouseEvent *event) {
...
void CanvasBase::mouseMoveEvent(QMouseEvent *event) {
    // Use normal event propagation
    QWidget::mouseMoveEvent(event);
}

void CanvasBase::mouseReleaseEvent(QMouseEvent *event) {
    // Use normal event propagation
